          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {loading ? (
              Array.from({ length: 4 }).map((_, index) => (
                <div key={index} className="min-w-[300px] bg-white h-[450px] shadow-xl rounded animate-pulse">
                  <div className="w-full h-60 bg-gray-300 rounded mb-4"></div>
                  <div className='h-35 w-12 absolute top-0 right-4 bg-gray-400 rounded-b-lg'></div>
                  <div className='px-4'>
                  ...
                  <div className="h-10 bg-gray-400 rounded mt-2 mx-4"></div>
                </div>
              ))
            ) : (
              data.map((card) => (
                <div key={card._id} className="min-w-[300px] bg-white h-[450px] shadow-xl rounded">
